		...
		<div class="row">
			...
			<div class="col-sm-10  col-md-10 col-sm-offset-1 col-md-offset-1">
				...
				<div class="row">
					...
					<button type="button" class="btn btn-danger btn-lg col-lg-4 col-md-4 col-sm-4" onclick="confirmPre()">Confirm purchase</button>
				</div>
			</div>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		function buyConfirm(productsId,productsCounts) {
			...
			html +='<th>Total amount：</th>'+
					...
					'<button class="btn btn-danger col-sm-4 col-md-4 col-lg-4" onclick="addToShoppingRecordsPre(['+productsId+'],['+productsCounts+'])">Confirm purchase</button>'+
					'</div>'+
					'</div>';
			layer.open({
				type:1,
		...
		function addToShoppingRecordsPre(productsId,productsCounts) {
			...
			layer.confirm('Go to order status？', {icon: 1, title:'Purchase success',btn:['Go to order','Contunue Purchase ']},
					function(){
		...
		function addToShoppingRecords(productId,productCounts) {
			...
			if(buyResult == "success") {
				...
				layer.msg("Commodity "+product.name+" Purchase success",{icon:1});
			}
			else if(buyResult == "unEnough"){
				layer.alert("Commodity "+product.name+" Lack of inventory, failed to purchase")
			}
		}
